...
export class Keyboard
{
  ...
  CreateReadLine()
  {
    process.stdin.setRawMode(true); // Make stdin not wait for enter to be pressed
  ...
  CreateListeners()
  {
    this.rl.on("line", this.on_line = this.OnLine.bind(this));
    this.rl.on("history", this.on_history = this.OnHistory.bind(this));
    this.rl.on("pause", this.on_pause = this.OnPause.bind(this));
    this.rl.on("resume", this.on_resume = this.OnResume.bind(this));
    this.rl.on("close", this.on_close = this.OnClose.bind(this));

    process.stdin.on("keypress", this.on_key = this.OnKey.bind(this));
  }

  IsPaused(){ return this.rl.paused; }
    ...
    else
    {
      switch (line)
      {
        ...
        default:
        {
          // try
          // {
          //   const fn = new global.Function(line);
          //   fn();
          // }
          // catch (error)
          // {
  ...
  OnClose(input)
  {
    this.rl.removeListener("line", this.on_line);
    this.rl.removeListener("history", this.on_history);
    this.rl.removeListener("pause", this.on_pause);
    this.rl.removeListener("resume", this.on_resume);
    this.rl.removeListener("close", this.on_close);

    process.stdin.removeListener("keypress", this.on_key);

    delete this.on_line;
    delete this.on_history;
    delete this.on_pause;
    delete this.on_resume;
  ...
  {
    this.#capturing = true;
    this.#prompt_resolve = undefined;
    this.#prompt_reject = undefined;

    return new Promise((resolve, reject) =>
    {
      this.#prompt_resolve = resolve;
      this.#prompt_reject = reject;
    });
  }

  ClearLine(direction = 0){ readline.clearLine(global.process.stdout, direction); return this; }

  Write(string){ global.process.stdout.write(string); return this; }
  GoTo(l = 0, c = 0){ return this.Write(`\u001b[${l};${c}H`); }
  Up(n = 1){ return this.Write(`\u001b[${n}A`); }
  Down(n = 1){ return this.Write(`\u001b[${n}B`); }
  Forward(n = 1){ return this.Write(`\u001b[${n}C`); }
  Back(n = 1){ return this.Write(`\u001b[${n}D`); }
  EraseToEnd(){ return this.Write(`\u001b[K`); }
  Save(){ return this.Write(`\u001b[s`); }
  Load(){ return this.Write(`\u001b[u`); }

  OnKey(raw, key)
  {
    key.alt = key.meta;
    key.raw = raw;
    // console.log(this.#capturing, "OnKey", raw, key);
