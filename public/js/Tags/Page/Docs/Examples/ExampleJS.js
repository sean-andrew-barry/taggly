...
export class Styles extends Attributes
{
  ...
  GridColumnGap(v, i){ return this.SetStyle("grid-column-gap", this.constructor.GetSize(v), i); }
  GridRowGap(v, i){ return this.SetStyle("grid-row-gap", this.constructor.GetSize(v), i); }
  // GridTemplateAreas(v, i){ return this.SetStyle("grid-template-areas", `${v.map(row => `"${row}"`).join("\n")}`, i); }
  ...
  // Rotate(v, i){ return this.AppendFrames("transform", v.map(v => `rotate(${this.constructor.ToDegree(v)})`)); }

  // Translate3D(...a){ return this.AppendFrames("transform", a.map(v => `translate3d(${v})`)); }
  ...
  FontSize(v, i){ return this.SetStyle("font-size", v, i); }
  ...
  Flexible(i){ return this.DisplayFlex(c, i).AlignItemsCenter(c, i); }
