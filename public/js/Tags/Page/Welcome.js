...
import { Tag, Page, Docs, Div, Section, P, Nav, Link, Span, Img, A, I, H1, H2, H3, H4, H5, H6, Code as CodeBase, Pre, Em, OL, UL, LI, Strong, Fragment, Form, Input, Label, Style, CSS, Number, Template, BR } from "/js/Tags.js";
import { OnConnect, OnMouseOver, OnMouseOut, OnInput, OnClick } from "/js/Events.js";
...
export class Welcome extends Docs {
  ...
  Div() { return Code.Text("Div"); }
  Import() { return Code.Text("import"); }
  Layers() { return Code.Text("layers"); }
  Layer() { return Code.Text("layer"); }
  Domains() { return Code.Text("domains"); }
  Domain() { return Code.Text("domain"); }
  Public() { return Code.Text("public"); }
  Private() { return Code.Text("private"); }
  JS() { return Code.Text("js"); }
  Tags() { return Code.Text("Tags"); }
  Page() { return Code.Text("Page"); }
  Welcome() { return Code.Text("Welcome.js"); }
  Specifier() { return Code.Text(this.SpecifierPath()); }
  Sample1Code() { return Pre.Text(SAMPLE_1.trim()); }
  Sample1() { return Code.Text("GetSample1()"); }
  Sample2() { return Code.Text("GetSample2()"); }
  HelloWorld() { return Code.Text(`"${this.GetSample1() ?? "Hello world!"}"`); }
  ...
  RootPath() {
    return Span.Class("root_path").Text(this.file_path_input?.GetValue().replace(/^\/|\/$/g, "") ?? `C:/Users/Sean/Documents/`);
  }

  SpecifierPath() { return `/js/Tags/Page/Welcome.js`; }

  Website() {
    return Span.Class("website_name").Text(this.website_name_input?.GetValue().replace(/^\/|\/$/g, "") ?? "MyWebsite");
  }

  CtrlQ() { return Code.Text("ctrl-q"); }
  Next() { return Code.Text("next"); }

  FullPathPublic() {
    return new Code().OverflowWrapBreakWord().WhiteSpacePreLine().Append(
      this.RootPath(),
      this.Website(),
      "/",
      Strong.Text(`public`),
  ...
  get layers() { return Code.Text("layers"); }
  get layer() { return Code.Text("layer"); }
  get tag() { return Code.Text(`Tag`); }
  get taggly() { return Code.Text(`Taggly`); }
  get loader() { return Code.Text(`Loader`); }
  get import() { return Code.Text(`import`); }
  get import_welcome() { return Code.Text(`import {Welcome} from "/js/Tags/Page/Welcome.js";`); }
  get not() { return Strong.Text("not"); }
  get next() { return Code.Text("next"); }
  get welcome() { return Code.Text("Welcome.js"); }
  get specifier() { return Code.Text(this.SpecifierPath()); }
  get domains() { return Code.Text("domains"); }
  get domain() { return Code.Text("domain"); }
  get public() { return Code.Text("public"); }
  get private() { return Code.Text("private"); }
  get node_modules() { return Code.Text("node_modules"); }
  get package_json() { return Code.Text("package.json"); }
  get js() { return Code.Text("js"); }
  get tags() { return Code.Text("Tags"); }
  get page() { return Code.Text("Page"); }
  ...
  get div() { return Code.Text("Div"); }
  get dom() { return Code.Text("DOM"); }
  get node() { return Code.Text("Node"); }
  get example_3() { return Code.Text("Example3"); }

  get root_path() { return Span.Class("root_path").Text(this.GetWebsitePath()); }
  ...
  get refresh() {
    return A.Text("refresh").OnClick(e => {
  ...
  Section(...tags) {
    return Section.Class("section").Append(
      Div.Class("container").Append(
        Div.Class("content is-medium").Append(
          ...tags,
        ),
      ),
    );
  ...
  SubSection(...tags) {
    return Section.Class("sub-section").Append(
      ...tags,
    );
  }
  ...
  RenderTextField(name, label, value, selector) {
    return Div.Class("field").Append(
      label && Label.Class("label is-medium").Text(label),
  ...
  Introduction() {
    return this.Section(
      H1.Class("title").Text("Welcome to Taggly"),
      H3.Class("subtitle").Text("Ready to get started?"),
  ...
  TheFileSystem() {
    return this.Section(
      ...
      OL.Append(
        LI.Append(this.private),
        LI.Append(this.public),
      ),

      P.Append`Each layer refers to an actual directory in your computer's filesystem, and each domain refers to a sub-directory in each layer. If you take a look in ${this.taggly}'s actual filesystem (usually you will have it installed in your ${this.node_modules} directory), you will see that it does indeed have these two ${this.public} and ${this.private} directories.`,

      P.Append`Here's a visualization of the layer system.`,

      this.VisualizeLoader(),

      P.Append`When the ${this.loader} gets a specifier from an import statement, like ${this.div_specifier}, it begins by iterating over each layer, and then each domain in each layer. It will check if there is a file at each of these paths:`,

      OL.Append(
        LI.TL(Code.Append`${this.root_path}${this.website}/private${this.div_specifier_string}`),
        LI.TL(Code.Append`${this.root_path}${this.website}/public${this.div_specifier_string}`),
        LI.TL(Code.Append`${this.root_path}taggly/private${this.div_specifier_string}`),
        LI.TL(Code.Append`${this.root_path}taggly/public${this.div_specifier_string}`),
      ),
  ...
  CreatingYourOwnFiles() {
    return this.Section(
      ...
      P.Append(
        Span.TL`Okay, let's actually get started writing some code! The first thing you should do is create your two ${this.domain} subdirectories in your project's root folder. For example, if your project is setup at ${Code.Append(this.root_path, this.website)} then you should create two new directories, at `,

        UL.Append(
          LI.Append(Code.TL`${this.root_path}${this.website}/public`),
          LI.Append(Code.TL`${this.root_path}${this.website}/private`),
        ),
      ),
      ...
      `If your root directory is at ${Code.Append(this.RootPath(), this.Website())} then the complete file path it checks would be ${this.FullPathPrivate()}.`,
      ...
      fn => {
        ...
        if (example_3_result) {
          ...
          fn.Append(
            ...
            Pre.TL`return new Div().Class("box").CursorCrosshair().BackgroundColor("#cecece").Append(\n  "${text}",\n  42,\n);`,
    ...
    return this.SubSection(
      ...
      Pre.Text(function () {
        P.Append(
          ...
          Code.Text("Welcome.js"),
          `. Specifically, let's look at this line: `,
          Code.Text(`import {Welcome as Base} from "/js/Tags/Page/Welcome.js?next=${this.website}";`),
          `. That `,
          Code.Text("import"),
      ...
      Div.Class("notification is-info").Append(
        P.TL`The raw HTML generated looks like this: ${Pre.Append(this.paragraph_target?.RemoveAttribute("id").ToPrettyHTML())}`,
      ),
      ...
      new Pre().OverflowWrapBreakWord().WhiteSpacePreLine().Text(function () {
        P.TL`Well, let's go back to the original code I asked you to copy in to ${Code.Text("Welcome.js")}. Specifically, let's look at this line: ${Code.Text(`import {Welcome as Base} from "/js/Tags/Page/Welcome.js?next=${this.website}";`)}. That ${Code.Text("import")} statement is handled by the ${Code.Text("loader")}. It tells the ${Code.Text("loader")} to search for a file called ${Code.Text("Welcome.js")} in the path ${Code.Text(`/js/Tags/Page`)}.`;
      }),
      ...
      new Pre().OverflowWrapBreakWord().WhiteSpacePreLine().Text(function () {
        P.TL`Well, let's go back to the original code I asked you to copy in to ${this.welcome}. Specifically, let's look at this line: ${this.welcome_import}. That ${this.import} statement is handled by the ${this.loader}. It tells the ${this.loader} to search for a file called ${this.welcome} in the path ${this.page_specifier}.`;
      }),
  ...
  SubSectionStaticConstructors() {
    return this.SubSection(
      ...
      UL.Append(
        ...
        LI.TL`${Code.TL`P.Append(...)`} instead of ${Code.TL`new P().Append(...)`}`,
  ...
  RenderVisualLayer(name, index, domains = ["private", "public"]) {
    return Div.Class("columns is-centered").Append(
      Div.Class("column is-8").Append(
        Div.Class("box").Append(
          H2.Append("Layer ", index + 1, ": ", name),

          Div.Class("columns is-centered").Append(
            ...domains.map((d, i) =>
              Div.Class("column is-5").Append(
                Div.Class("box has-background-primary").Append(
                  P.Class("has-text-white").Append("Domain: ", i + 1, ": ", d),
                ),
              ),
            ),
          ),
  ...
  VisualizeLoader(layers = [this.website, "Taggly"], domains) {
    ...
    return Fragment.Append(
      Div.Class("content is-large has-text-centered").Append(
  ...
  RenderBody() {
    return super.RenderBody(
      ...
      () => this.Introduction(),
      () => this.TheFileSystem(),
      () => this.CreatingYourOwnFiles(),
      () => this.Summary(),
      () => this.SectionUsingTags(),
    );
  }
}